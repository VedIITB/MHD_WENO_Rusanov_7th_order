...
void RK3()
{
 ...
 for(j=8; j<=size-12; j=j+2)
    ...
    for(k=0; k<8; k++)
    {
   	...
   	U_1[k][i+2][j+2] = U[k][i+2][j+2] + (delta_t/delta_x)*(F1[k][i+2][j+1] - F1[k][i+2][j+3]) + (delta_t/delta_x)*(G1[k][i+1][j+2] - G1[k][i+3][j+2])
	- (delta_t)*(S[k][i+2][j+2]);
...
iteration();
 ...
 for(j=8; j<=size-12; j=j+2)
    ...
    for(k=0; k<8; k++)
    {
   	...
   	U_new2[k][i+2][j+2] = 0.33*U[k][i+2][j+2] +   0.67*U_new[k][i+2][j+2] + (0.67*delta_t/delta_x)*(F1[k][i+2][j+1] - F1[k][i+2][j+3]) + (0.67*delta_t/delta_x)*(G1[k][i+1][j+2] - G1[k][i+3][j+2])
	+(0.67*delta_t)*(S[k][i+2][j+2]);
}
...
if(i==50 && j==50)
cout<<"EVOLVING FOR TIME "<<t<<"\t"<<density[i+2][j+2]<<"\n";
...
for(i=10; i<=size-10; i=i+2)
    {
  		...
  		pressure[i][size-8]     =   pressure[i][size-6];
        density[i][size-8]     =   density[i][size-6];
        ethalpy[i][size-8]      =  ethalpy[i][size-6];
        velocity_x[i][size-8]    =  velocity_x[i][size-6];
		velocity_y[i][size-8]    =  velocity_y[i][size-6];
		velocity_z[i][size-8]    = 	velocity_z[i][size-6];
          magnetic[i][size-8]    =   magnetic[i][size-6];
          Bz[i][size-8]          =  Bz[i][size-6] ;
          bx[i][size-8]          =  bx[i][size-6] ;
	    speed_sound[i][size-8]=     speed_sound[i][size-6];           
...
for(i=10; i<=size-10; i=i+2)
    {
	 	...
	 	pressure[i][n5]     =    pressure[i][nj];
        density[i][n5]      =    density[i][nj];
        ethalpy[i][n5]      =    ethalpy[i][nj];
        velocity_x[i][n5]   =   velocity_x[i][nj]  ;
		velocity_y[i][n5]   =   velocity_y[i][nj] ;
		velocity_z[i][n5]   = 	 velocity_z[i][nj] ;
          magnetic[i][n5]   =   magnetic[i][nj];
          Bz[i][n5]         =   Bz[i][nj]  ;
           bx[i][n5]        =  bx[i][nj]  ;
	   speed_sound[i][n5]   =  speed_sound[i][nj] ;          
